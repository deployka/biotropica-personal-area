...
import {addMessage, updateMessage} from "../../../../store/ducks/chat/actionCreators";
...
export const InnerChat = ({options, onClose, id}: Props) => {
    ...
    useEffect(() => {
        chatWsService.addEventListener(WSEvent.onMessageReceived, handleOnMessageReceived)
        return () => chatWsService.removeEventListener(WSEvent.onMessageReceived, handleOnMessageReceived)
    }, [])
    ...
    const List = list.map(item => {
        switch (item.type) {
            case "message":
                ...
                />
            case 'timemark':
                return <Timemark time={item.patload} key={`timemark-${item.patload}`}/>
        }
    });
