...
export const eventTaskOptions: SelectOptions<EventTask['kindOfEvent']>[] = Object.keys(
  translatedKindOfEvent,
).map((key: string) => {
  return {
...
export const selectRepeatType: SelectOptions<EventTask['repeatType']>[] = Object.keys(
  translatedRepeatType,
).map((key: string) => {
  return {
...
export const translatedCompletionType: Record<EventTask['completionType'], string> = {
  byDate: 'До определенной даты',
  byRepetitionsNumber: 'Количество повторений',
};

export const selectCompletionType: SelectOptions<EventTask['completionType']>[] = Object.keys(
  translatedCompletionType,
).map((key: string) => {
  return {
